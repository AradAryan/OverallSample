    ...
    $("#grid").kendoGrid({
        ...
        dataBound: function () {
            ...
            for (var i = 0, uid; i < data.length; i++) {
                if (data[i].EtebariUserOK == true) {
                    uid = data[i].uid;
                    $('tr[data-uid=' + uid + ']').addClass('text-success');
                    $('tr[data-uid=' + uid + ']').children('td').eq(7).children('a').remove();
                    $('tr[data-uid=' + uid + ']').children('td').eq(6).children('textarea').remove();
                    $('tr[data-uid=' + uid + ']').children('td').eq(6).append('<p class="text-primary">' + data[i].EtebariUserExplain + '</p>');
                    $('tr[data-uid=' + uid + ']').children('td').eq(7).append('<p>تایید شده</p>');
                }
                else if (data[i].EtebariUserOK == false) {
                    uid = data[i].uid;
                    $('tr[data-uid=' + uid + ']').addClass('text-danger');
                    $('tr[data-uid=' + uid + ']').children('td').eq(7).children('a').remove();
                    $('tr[data-uid=' + uid + ']').children('td').eq(6).children('textarea').remove();
                    $('tr[data-uid=' + uid + ']').children('td').eq(6).append('<p class="text-primary">' + data[i].EtebariUserExplain + '</p>');
                    $('tr[data-uid=' + uid + ']').children('td').eq(7).append('<p>رد شده</p>')
                }
            }
        }
