function initVM(args) {
    var vm = kendo.observable({
        ...
        onAgree: function (e) {
            ...
            var tozih = $(e.currentTarget).closest('tr').children('td').eq(7).children('textarea').val();
        ...
        AgreeDS: new kendo.data.DataSource({
            transport: {
                read: {
                    type: 'POST',
                    dataType: "json",
                    url: 'http://localhost/UI/Cartable/AgreedByMaaliUser',
                    data: {
        ...
        disagreeDS: new kendo.data.DataSource({
            transport: {
                read: {
                    type: 'POST',
                    dataType: "json",
                    url: 'http://localhost/UI/Cartable/RejectedByMaaliUser',
                    data: {
                        loanId: function () { return vm.selectedItemId },
                        tozihat: function () { return vm.tozihat }
                    }
